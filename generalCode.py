            ...
            if ch==1:
                ...
                while True:
                    if tmp in data or rno not in ro:
                        print(colorText("[[red]]This room number has been already booked or not exists![[white]]"))
                        rno=int(input("Enter the room number: "))
                        tmp = (rno,)
                    else:
                        break
                
                mycur.execute("UPDATE statuss SET status_room = {0} WHERE Room_no = {1}".format(1, rno))
                cn.commit()
                ...
                if rno == 101:
                    s=no*6000
                    q="insert into Cust_data values('{0}','{1}','{2}','{3}','{4}','{5}')".format(rno,n,add,no,'A',s)
                    mycur.execute(q)
                    cn.commit()
                elif rno == 102:
                    s=no*5000
                    q="insert into Cust_data values('{0}','{1}','{2}','{3}','{4}','{5}')".format(rno,n,add,no,'B',s)
                    mycur.execute(q)
                    cn.commit()
                elif rno == 103:
                    s=no*4000
                    q="insert into Cust_data values('{0}','{1}','{2}','{3}','{4}','{5}')".format(rno,n,add,no,'C',s)
                    mycur.execute(q)
                    cn.commit()
                elif rno == 104:
                    s=no*3000
                    q="insert into Cust_data values('{0}','{1}','{2}','{3}','{4}','{5}')".format(rno,n,add,no,'D',s)
                    mycur.execute(q)
                    cn.commit()
                ...
                while x!=6:
                    ...
                    if x==1:
                        ...
                        n=int(input("\tEnter No. of Hours: "))
                        q="Update Services_bill set Num_Outdoorgym='{0}' where room_no = '{1}'".format(n,rno)
                        A='Outdoor Gym'
                        sr=sr+n*680
                        mycur8.execute(q)
                ...
                for x in data:
                    ...
                    for y in range(0,len(x)):
                        if y==0:
                            ...
                            if x[y] >= 200000:
                                print(colorText('[[red]]|[[white]]'), "10%", end='     \t')
                                res = x[y]
                                res -= res * decimal.Decimal(0.1)
                            elif x[y] >= 160000:
                                print(colorText('[[red]]|[[white]]'), "20%", end='     \t')
                                res = x[y]
                                res -= res * decimal.Decimal(0.2)
                            elif x[y] >= 120000:
                                print(colorText('[[red]]|[[white]]'), "30%", end='     \t')
                                res = x[y]
                                res -= res * decimal.Decimal(0.3)
                            else:
                                print(colorText('[[red]]|[[white]]'), "0", end='     \t')
                                res = x[y]
                            print(colorText('[[red]]|[[white]]'), int(res), end='     \t')
                        else:
                            print(colorText('[[red]]|[[white]]'), x[y], end='     \t')
                ...
                # paid
                mycur10.execute("DELETE FROM amusement_bill WHERE Room_no = {0}".format(rno))
                mycur10.execute("DELETE FROM bar_bill WHERE Room_no = {0}".format(rno))
                mycur10.execute("DELETE FROM carrent_bill WHERE Room_no = {0}".format(rno))
                mycur10.execute("DELETE FROM cust_data WHERE Room_no = {0}".format(rno))
                mycur10.execute("DELETE FROM food_bill WHERE Room_no = {0}".format(rno))
                mycur10.execute("DELETE FROM game_bill WHERE Room_no = {0}".format(rno))
                mycur10.execute("DELETE FROM laundry_bill WHERE Room_no = {0}".format(rno))
                mycur10.execute("DELETE FROM services_bill WHERE Room_no = {0}".format(rno))         
